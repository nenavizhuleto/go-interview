...
func (c column) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		c.list.SetSize(msg.Width, msg.Height/2)
	case tea.KeyMsg:
		switch {
		case key.Matches(msg, keys.Edit):
			if len(c.list.VisibleItems()) != 0 {
				todo := c.list.SelectedItem().(Todo)
				f := NewForm(todo.title, todo.description)
...
func (c *column) Set(i int, t Todo) tea.Cmd {
	if i != APPEND {
...
func (c *column) MoveToNext() tea.Cmd {
	...
	if task, ok = c.list.SelectedItem().(Todo); !ok {
		return nil
	}
	// move item
	c.list.RemoveItem(c.list.Index())
